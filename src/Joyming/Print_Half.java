...
public class Print_Half implements Printable {
    ...
    public void printHalf(String filePath, PrinterJob printerJob) throws PrinterException, IOException {
        Book book = new Book();
        ...
        book.append(new Print_Half(), pageFormat);
...
//            boolean a = printerJob.printDialog();
            if (len == 0) {
                ...
                for (i = 1; i <= len; i += 2) {
                    System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss ：").format(System.currentTimeMillis()) + "两张为1页：第" + (i / 2 + 1) + "页开始打印");
                    try {
                    ...
                    System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss ：").format(System.currentTimeMillis()) + "两张为1页：第" + (i / 2 + 1) + "页打印结束咯");
                }
                System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss ：").format(System.currentTimeMillis()) + "*****两张为1页已经全部打印完成，共" + len + "页！！！*****\n\n");
            }
        } else {
